...
import { type FullEvent, fromSupabase } from './eventTransform'
...
export default function EventoPage({ initialEvent }: { initialEvent?: FullEvent | null }) {
  const params = useParams()
  const id     = String(params.id)
  const isUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(id)

  const [ev,             setEv]             = useState<FullEvent | null>(initialEvent ?? null)
