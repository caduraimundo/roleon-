            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ fontSize: 11, color: DIM, display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                {c.expires_at && new Date(c.expires_at) < new Date() && (
                  <span style={{ fontSize: 11, fontWeight: 600, color: '#FF3B30', background: '#FFF0F0', padding: '3px 8px', borderRadius: 20 }}>Expirado</span>
                )}
                {c.max_uses && c.uses_count >= c.max_uses && (
                  <span style={{ fontSize: 11, fontWeight: 600, color: '#FF3B30', background: '#FFF0F0', padding: '3px 8px', borderRadius: 20 }}>Esgotado</span>
                )}
              </div>
              <button
              ...
              {!c.active && c.locked_by_admin && (
                <span style={{ fontSize: 11, fontWeight: 600, color: '#FF3B30', background: '#FFF0F0', padding: '3px 8px', borderRadius: 20, marginLeft: 8 }}>Bloqueado por você</span>
              )}
            </div>
          </div>
        ))}
            ...
            {webhooks.map((w) => {
              ...
              return (
                <div key={w.id} style={{ background: WHITE, borderRadius: 10, padding: '12px 14px', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: 11, fontWeight: 600, padding: '3px 8px', borderRadius: 20, background: sBadge.bg, color: sBadge.fg }}>{sBadge.label}</span>
                    <span style={{ fontSize: 11, fontWeight: 600, padding: '3px 8px', borderRadius: 20, background: aBadge.bg, color: aBadge.fg }}>{aBadge.label}</span>
                  </div>
          ...
          {!prodLoading && filtered.map((p: any) => {
            ...
            return (
              <div key={p.id} onClick={() => openProdDetail(p)} style={{ background: WHITE, borderRadius: 12, border: `1px solid ${BORDER}`, padding: 14, marginBottom: 10, display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 600, background: sb.bg, color: sb.color, border: `1px solid ${sb.border}`, borderRadius: 20, padding: '3px 8px', whiteSpace: 'nowrap', flexShrink: 0 }}>{sb.label}</span>
              </div>
            )
          })}
        </div>
